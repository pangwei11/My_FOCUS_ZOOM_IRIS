...
//     ret = an41908_write_reg(dev, 0x01, 0x0000);     //这个低位先不用管都使用为0默认值，高七位是DGAIN[6:0]（PID 控制器增益），也就是PID中的P比例
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x02, 0x3500);     //设置PID零点 = 35Hz，极点 = 950Hz，PID_ZERO[3:0]其实就是PID中的I积分，PID_POLE[3:0]就是PID中的D微分
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x03, 0x0230);     //设置PID的PWM频率 = 31.25KHz
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x04, 0x8040);     //设设置霍尔的偏置电流与偏置电压
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x05, 0x0004);     //设设置霍尔的偏置电流与偏置电压
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x0A, 0x075F);     // 驱动信号占空比与 PWM_IRIS[2:0] 的设置值有关
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x0B, 0x0410);     // 
//     if (ret < 0) return ret;

//     ret = an41908_write_reg(dev, 0x0E, 0x0E50);     // 
//     if (ret < 0) return ret;
